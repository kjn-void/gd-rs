...
Benchmark sources: [Rust/Rayon `par_benchmark`](../../benches/par_benchmark.rs#L8-L120) ·
[C++/OpenMP `par_benchmark`](../../benches/cpp-reference/par_benchmark.cpp#L20-L128) ·
...
The programs are intentionally one-pass harnesses rather than adaptive Criterion or
Google Benchmark cases: repeating this exponential workload automatically would make
an already long run impractical. `GD_PAR_ROWS` and `GD_PAR_MAX_ARG` can reduce the
fixture for calibration; release execution without those variables uses the exact
100M-row, `1..=23` defaults. Cargo's debug/test execution automatically substitutes
400 rows and `1..=20`, so the repository's all-target test command remains bounded.
